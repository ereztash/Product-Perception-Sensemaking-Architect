...
Fourteen branches are currently ahead of `main` and nothing is retirable: the twenty-one
contained refs were deleted on 2026-09-08. Deletion is refused in every agent session by the
egress proxy, so it is an owner action: `docs/BRANCH_RETIREMENT_RUNBOOK.md`.
