...
Return exactly ONE JSON object and no Markdown with exactly: material_question (string), bottleneck (string), resource_assessment (non-empty array; each item exactly resource, expected_contribution, authority_ceiling, uncertainty strings), candidate_moves (non-empty array; each item exactly move, resource, expected_decision_value, reversibility strings), needs (object with exactly these boolean keys: signal_interpretation_ambiguity, multiple_plausible_mechanisms, proxy_substitution_risk, research_to_intervention_transition, broad_reasoning_needed, architecture_alternatives_needed, novel_synthesis_needed, external_research_needed, owner_authority_needed, repo_authority_needed, environment_authority_needed, field_authority_needed), rationale (string). Do not add fields. Diagnose this task, not examples in the prompt. You are running in a GitHub Actions workspace. When the task names local repository paths or datasets, inspect them directly with available filesystem/shell tools before finalizing the diagnosis; do not rely on the task summary when the underlying evidence is locally available."""
    if resource == "RND" and phase == "SYNTHESIZE":
        return """# Runtime bridge contract
Return exactly ONE JSON object and no Markdown with exactly: decision_before, decision_after, next_move, resource_deltas (array; each item resource, material boolean, unique_delta), learning_records (array of objects), stop_or_continue (STOP or CONTINUE), routing_amendment_proposed (normally null). Preserve disagreements and authority ceilings. Role-conditioned agreement is not independent triangulation. If local evidence paths are named in the task, inspect them directly when doing so can change the synthesis. Stop when another analysis pass is unlikely to change the prioritized user-facing output, next action, reversal condition, or evidence status."""
    if resource == "NETA" and phase == "ANALYZE":
        return """# Runtime bridge contract
Return exactly ONE JSON object and no Markdown with exactly: resource:"NETA", summary (string), unique_delta (string), evidence_refs (array of strings, only refs present in request), limitations (array of strings). Analyze the actual task and requested_focus independently. Challenge signal-to-interpretation jumps, mechanism collapse, proxy substitution, premature intervention, and user-facing language that outruns the evidence. When the task concerns a player's move-level evidence, distinguish observable chess patterns from cognitive explanations and help translate supported distinctions into language a player can act on. Do not infer FIELD behavior or causal outcome."""
