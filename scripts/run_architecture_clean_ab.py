...
import argparse, datetime as dt, hashlib, json, os, sys, time, subprocess, tempfile
...
ENGINE=os.getenv('ARCH_BENCH_ENGINE','openai')
MODEL=os.getenv('ARCH_BENCH_MODEL',os.getenv('OPENAI_MODEL','gpt-5.6-sol')) if ENGINE=='openai' else os.getenv('ARCH_COPILOT_MODEL','auto')
...
def call(prompt,inp,model,effort,max_tokens):
    ...
    for n in range(4):
        try:
            if ENGINE=='copilot':
                full=prompt+'\n\nFROZEN INPUT JSON:\n'+canon(inp)
                with tempfile.TemporaryDirectory(prefix='arch-ab-clean-') as td:
                    cp=subprocess.run(['copilot','-s','-p',full,'--no-custom-instructions','--no-ask-user','--model',model],cwd=td,text=True,capture_output=True,timeout=600)
                if cp.returncode: raise RuntimeError('copilot failed: '+cp.stderr[-1600:])
                sem=parse_json_object(cp.stdout)
                return sem,{'engine':'copilot','model_requested':model,'reasoning_effort':'copilot-managed'}
            ...
            return sem,{'engine':'openai','response_id':raw.get('id'),'model_requested':model,'model_actual':raw.get('model'),'reasoning_effort':effort,'usage':raw.get('usage')}
...
def run(outroot):
    ...
    s=summarize(judged); s.update({'run_id':runid,'freeze_manifest_sha256':msh,'engine':ENGINE,'model':MODEL,'judge_model':JUDGE_MODEL,'status':'COMPLETE_VISIBLE_TRAIN_CLEAN_RUNNER_BLIND_ADJUDICATION'}); write(out/'SUMMARY.json',s); print(json.dumps(s,ensure_ascii=False,indent=2)); return 0
